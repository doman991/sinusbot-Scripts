...
---
## Poke Bot
**Spam Poke Bot for SinusBot** allows you to repeatedly poke multiple users on a TeamSpeak server at set intervals. You can customize the message and time delay (in seconds) between pokes.

This is useful for notifications, alerts, or simply grabbing attention in a persistent manner.
---
## Multi Rang
...
- Added a reset command (`!resetexp` with confirmation via `!confirm`) available only to users from a configurable server group, which renumbers all channels sequentially starting from 1 and resets the internal counter.
